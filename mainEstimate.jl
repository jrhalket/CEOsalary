...
Compparams = (n_firms=length(FullData.up_data_obs[:,1]), n_sim=10, trim_percent=0, 
                    hmethod=4, nparams=14, logcompdum=0, dist=Normal);
...
Initparams = (n_firms=length(InitData.up_data_obs[:,1]), n_sim=10, trim_percent=0, hmethod=4, nparams=14, logcompdum=0, dist=Cauchy)
...
b_init[12]=log(mean(InitData.wages_obs));
b_init[13]=.01;
b_init[14]=.01;
...
b_init = parse.(Float64,split(last(eachline("Test0706.csv")),","));
#########
...
for i in 1:Compparams.nparams;
    ...
    nll = make_closures2(b_cal,i_cal,InitData,h,quasi_seed, Initparams;multithread=false)
        ...
        CSV.write("Test0706.csv", Tables.table(b_init'), append=true) 
...
resParams = parse.(Float64,split(last(eachline("Test0706.csv")),","));
