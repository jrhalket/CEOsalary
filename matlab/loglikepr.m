function ll = loglikepr(b_est, b_cal, i_cal, Data,h,LLC)
  ...
  elseif LLC.dist == 'Cauchy'
    for i= 1:LLC.n_firms
        for j = 1:LLC.n_sim
            like(i) = like(i) + exp(... 
                  -log(pi*(1+(Data.down_data_obs(i,1) - sim_dat(j).down_match(i,1))/h(1)).^2)...
                + -log(pi*(1+(Data.wages_obs(i) - sim_dat(j).wages_match(i))/h(2)).^2) ...
                + -log(pi*(1+(Data.measures_obs(i,1) - sim_dat(j).measures_match(i))/h(3)).^2)...
                );
